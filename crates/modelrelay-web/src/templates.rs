...
(function() {
  ...
  setInterval(pollAll, POLL_MS);
    ...
    let extra_css = vec!["<link rel="stylesheet" href="/static/css/dashboard_override_css.css">", "<link rel="stylesheet" href="/static/css/dashboard_css.css">"];
    ...
    let extra_body_end = vec!["<script src="/static/js/dashboard_js.js"></script>"];
...
pub fn integrate_page_with_config(cloud_config: Option<&CloudWizardConfig>) -> String {
    // Data-driven integration page - manifest loaded from crate
    let _cfg = cloud_config;
    // For now, render minimal shell with static assets
    let extra_css = vec!["<link rel=\"stylesheet\" href=\"/static/css/integrate_css.css\">", "<link rel=\"stylesheet\" href=\"/static/css/integrate_override_css.css\">"];
    let extra_body_end = vec!["<script src=\"/static/js/integrate_js.js\"></script>"];
    page_shell_custom(
        "Integrate",
        "/integrate",
        &integrate_body_from_manifest(),
        false,
        &extra_css.join(""),
        &extra_body_end.join(""),
    )
...
fn integrate_body_from_manifest() -> String {
    crate::integrate_renderer::render_integrate_page()
}
]
